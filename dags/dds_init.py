...
Предположим пример: Мы выгрузили инит в dds слой. Однако, в следующей дельте мы не получили никаких новых данных.
                    Тем не менее мы всё равно выгрузим в dds данные инита, так как поле `inserted_at` не обновилось,
...
def load_fact_table():
    ...
    update_flag_query2 = """UPDATE staging.mongo_orders SET flag = 'True' 
                            ...
                            """
    cursor = conn.cursor()
    cursor.execute("TRUNCATE TABLE dds.fact_table;")
...
def load_time_dim():
    ...
    insert_query = """INSERT INTO dds.Dim_Time (time_mark, year, month, day, time)
                    ...
                    """
    update_flag_query = """UPDATE staging.api_delivery SET flag = 'True' 
                            WHERE flag = 'False';"""
    cursor = conn.cursor()
    cursor.execute("TRUNCATE TABLE dds.dim_time;")
    cursor.execute(insert_query)
...
def load_clients_dim():
    ...
    update_flag_query = """UPDATE staging.mongo_clients SET flag = 'True' 
                        ...
                        """
    cursor = conn.cursor()
    cursor.execute("TRUNCATE TABLE dds.dim_clients;")
    cursor.execute(insert_query)
    cursor.execute(update_flag_query)
    conn.commit()
    cursor.close()
...
def load_restaurants_dim():
    ...
    update_flag_query = """UPDATE staging.mongo_restaurants SET flag = 'True' 
                        ...
                        """
    cursor = conn.cursor()
    cursor.execute("TRUNCATE TABLE dds.dim_restaurants;")
    cursor.execute(insert_query)
    cursor.execute(update_flag_query)
    conn.commit()
    cursor.close()
...
def load_orders_dim():
    ...
    insert_query = """INSERT INTO dds.Dim_Orders (order_id, payed_by_bonuses, cost, payment, bonus_for_visit, final_status)
                    ...
                    """
    update_flag_query = """UPDATE staging.mongo_orders SET flag = 'True' 
                            WHERE flag = 'False';
                        """
    cursor = conn.cursor()
    cursor.execute("TRUNCATE TABLE dds.dim_orders;")
    cursor.execute(insert_query)
    cursor.execute(update_flag_query)
    conn.commit()
    cursor.close()
